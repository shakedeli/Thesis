...
% In the supplementary material, we show that $E[H_1] \leq 1.4$ for all $b \in \mathds{N}$.
% We calculate $H_1$ numerically and get, for $b=16$ for example, that $E\left[H_1\right] < 0.0386 \cdot 16 = 0.617$.


% \paragraph*{Analysis of $\boldsymbol{H_j}$.} For region $j$ ($j>0$), again, we first assume a scenario in which $T_O$ increments the index when $T_j$ hasn't copied any element from its local buffer to region $j$. Before reading this region, $T_O$ executes $b$ writes to copy its local buffer and then $(j-1)b$ reads of first $j-1$ regions.
% %For $j=2$, it must execute $b$ writes and $b$ reads of slot $1$. For $j=3$, it must execute $b$ writes and $2b$ reads of slots $1-2$, etc.
% Following the same analysis as before, the probability that at least $1$ hole is read in region $j$ is bounded by:
% \[P_j \leq b \cdot \left(\frac{1}{2}\right)^{2b + jb -1} {{2b+jb-2} \choose b-1} \]
% And, as before: 
% \[ E\left[H_j\right] \leq b\cdot P_j. \]
...
On the other hand, 
\begin{flalign*}
\left(\phi-\epsilon_r\right)n_{old} \geq &\left(\phi-\epsilon_r\right)\frac{n_{new}}{\rho}= &&\\
